...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuite with ShouldMatchers with BeforeAndAfterAll with ImplicitSender 
{
  ...
  test("remove works") {
    ...
    val ops = List(
      Insert(requester.ref, id=1, 1),
      Insert(requester.ref, id=2, 2),
      Remove(requester.ref, id=3, 3),
      Contains(requester.ref, id=4, 3)
    )
  ...
  test("insert works") {
    ...
    val ops = List(
      Insert(requester.ref, id=1, 1),
      Insert(requester.ref, id=2, 2),
      Insert(requester.ref, id=3, 3),
      Insert(requester.ref, id=4, 4)
    )
