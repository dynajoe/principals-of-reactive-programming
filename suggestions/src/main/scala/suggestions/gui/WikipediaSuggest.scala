...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.flatMap(x => wikiSuggestResponseStream(x).recovered)
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.flatMap(x => wikiPageResponseStream(x).recovered)
